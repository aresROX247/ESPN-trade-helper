...
const { findInstalledBrowser } = require('./scripts/environment');
...
// Browser detection lives in scripts/environment.js so setup, the CLI, and the
// importer always agree on what is installed (Windows, macOS, and Linux).
...
async function pullLeague(page) {
  ...
  const endpointPath = `/apis/v3/games/ffl/seasons/${season}/segments/0/leagues/${leagueId}?view=mSettings&view=mTeam&view=mRoster&view=mMatchup&view=mAvailablePlayers&view=mPendingTransactions`;
  const endpoints = [
  ...
  const playersEndpoint = `https://fantasy.espn.com/apis/v3/games/ffl/seasons/${season}/players?scoringPeriodId=${data.scoringPeriodId || 1}&view=players_wl&limit=1000`;
  try {
    const playersPage = await page.context().newPage();
    const playersResponse = await playersPage.goto(playersEndpoint, { waitUntil: 'domcontentloaded', timeout: 30000 });
    const playersBody = playersResponse ? await playersResponse.text() : '';
    if (playersResponse?.ok() && playersBody.trim().startsWith('{')) {
      const playersData = JSON.parse(playersBody);
      data.availablePlayers = playersData.players || [];
      console.log(`Pulled ${data.availablePlayers.length} available ESPN players.`);
    }
    await playersPage.close();
  } catch (error) {
    console.warn(`Could not pull available ESPN players: ${error.message}`);
...
async function main() {
  ...
  if (!browser) {
    throw new Error('No supported browser was found. Run "npm run setup" to install the bundled Chromium, or install Microsoft Edge or Google Chrome.');
  }
  console.log(`Using ${browser.name}.`);
  const context = await chromium.launchPersistentContext(PROFILE_DIR, { executablePath: browser.executablePath, headless: false, args: ['--start-minimized'] });
