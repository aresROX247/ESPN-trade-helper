...
const server = http.createServer(async (request, response) => {
  if (request.method === 'POST' && request.url === '/api/browser-import/start') {
    ...
    importerOutput = [];
    browserImporter = spawn(process.execPath, [path.join(__dirname, 'browser-import.js')], { cwd: __dirname, windowsHide: false });
    ...
    browserImporter.on('close', (code) => importerOutput.push(code === 0 ? 'Import complete.' : 'Import stopped with an error.'));
