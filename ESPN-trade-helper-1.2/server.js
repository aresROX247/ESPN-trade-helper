...
async function requestAiTradeAnalysis({ apiKey, payload }) {
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: 'You are a fantasy football trade analyst. Use only the supplied players. Never invent players, stats, injuries, or roster rules. Return JSON with keys summary, tradeIdeas, waiverTargets. Each tradeIdeas item must have offer, receive, fit, whyItHelps, whyTheyAccept, risk. Each waiverTargets item must have player, action (TARGET, WATCH, or PASS), reason, fit. Keep recommendations concise and explain that they are decision support, not certainty.' },
  ...
  try { return JSON.parse(responseBody.choices?.[0]?.message?.content || '{}'); } catch { throw new Error('AI returned an unreadable trade analysis.'); }
}
