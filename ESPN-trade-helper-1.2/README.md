...
- Direct ESPN Fantasy league URL importing with Playwright
- Uses the local authenticated browser session instead of manual cookie entry
...
- Includes an optional AI trade analyst and waiver-wire sidebar
...
The AI panel is disabled until you add your own OpenAI API key. The key is stored only in this browser's local storage and sent to the local server when you choose **Use AI analysis**. It is not written to `league-data.json` or logged by the app.

The waiver sidebar uses ESPN's available-player pool and provides rule-based `TARGET`, `WATCH`, or `PASS` guidance based on roster needs, projections, and last-season points. AI suggestions are decision support, not guaranteed trade advice.
...
- Saved league profiles contain the league name, season, and exact URL; ESPN cookies are never saved by the viewer.
